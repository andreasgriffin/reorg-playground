		...
		>
			...
			>
				<SheetHeader className="border-b border-border/80 pb-2">
					...
					<SheetTitle className="text-base tracking-tight sm:text-lg">
						Height #{block.height}
					</SheetTitle>
					<SheetDescription title={block.hash}>
						<MempoolHashLink
							network={network}
							hash={block.hash}
							itemType="block"
							className="inline-flex items-center gap-1.5 truncate font-mono text-xs text-muted-foreground underline-offset-4 hover:text-primary hover:underline"
							title={block.hash}
						>
				...
				<div className="flex-1 space-y-4 p-3">
					<section className="space-y-1.5">
						...
						<dl className="divide-y divide-border/60 border-y border-border/60">
							...
							<FieldRow
								label="Difficulty"
								value={String(block.difficulty_int)}
								mono
							/>
						</dl>
					</section>
						...
						<div className="divide-y divide-border/60 border-y border-border/60">
							<CopyableRow
								...
								link={
									<MempoolHashLink
										...
										hash={block.hash}
										itemType="block"
										className="mt-0.5 inline-flex items-center gap-1.5 break-all font-mono text-xs text-primary underline-offset-4 hover:underline"
										title={block.hash}
									/>
								}
							/>
							<CopyableRow
								...
								link={
									<MempoolHashLink
										network={network}
										hash={block.prev_blockhash}
										itemType="block"
										className="mt-0.5 inline-flex items-center gap-1.5 break-all font-mono text-xs text-primary underline-offset-4 hover:underline"
										title={block.prev_blockhash}
									/>
								}
							/>
							<CopyableRow
								...
								{block.tipStatuses.map((tipStatus) => (
									...
									>
										<span
											...
											style={{
												backgroundColor: TIP_STATUS_COLORS[tipStatus.status],
											}}
										/>
										<div className="min-w-0">
											<p className="text-sm font-semibold text-foreground">
												{STATUS_LABELS[tipStatus.status]}
											</p>
											<p className="wrap-break-word text-xs text-muted-foreground">
												{tipStatus.nodeNames.join(", ")}
											</p>
										</div>
									</li>
								))}
