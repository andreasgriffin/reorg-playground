...
function computeFoldSegments(
	...
	for (const block of blocks) {
		if (
			forkHeights.has(block.height) ||
			forkHeights.has(block.height - 1) ||
			forkHeights.has(block.height + 1)
		) {
...
type TraversalFrame =
	| { blockId: number; stage: "enter" }
	| { blockId: number; stage: "exit"; childIds: number[] };
...
function compactSlots(
	...
	for (const height of heights) {
		...
		atHeight.sort(
			(a, b) => (originalSlots.get(a.id) ?? 0) - (originalSlots.get(b.id) ?? 0),
		);
	...
	for (let h = heights.length - 1; h >= 0; h--) {
		...
		atHeight.sort(
			(a, b) => (newSlot.get(a.id) ?? 0) - (newSlot.get(b.id) ?? 0),
		);

		for (let i = 0; i < atHeight.length; i++) {
			const block = atHeight[i];
			const childIds = (childrenMap.get(block.id) ?? []).filter((id) =>
				newSlot.has(id),
			);
...
function getRootBlocks(
	...
	return blocks
		.filter(
			(block) => block.prev_id === MAX_PREV_ID || !blockMap.has(block.prev_id),
		)
		.sort(compareBlocks);
}
	...
	function assignSlot(startBlockId: number): number {
		...
		while (stack.length > 0) {
			...
			if (frame.stage === "enter") {
				...
				const childIds = (childrenMap.get(frame.blockId) ?? []).filter(
					(childId) => blockMap.has(childId),
				);
...
function createNextMineSlotResolver(
		...
		while (stack.length > 0) {
			...
			if (frame.stage === "enter") {
				...
				if (resolvingSubtree.has(frame.blockId)) {
					subtreeBoundsByBlockId.set(frame.blockId, {
						min: ownSlot,
						max: ownSlot,
					});
					continue;
	...
	return (blockId: number): number => {
		...
		const existingChildIds = (childrenMap.get(blockId) ?? []).filter(
			(childId) => blockMap.has(childId),
		);
		if (existingChildIds.length === 0) return parentSlot;

		const maxChildSubtreeSlot = Math.max(
			...existingChildIds.map((childId) => getSubtreeBounds(childId).max),
		);
		return Number.isFinite(maxChildSubtreeSlot)
			? maxChildSubtreeSlot + 1
			: parentSlot;
	};
}
 ...
 */
function createDepthLookup(
	visibleBlocks: ProcessedBlock[],
	collapsedSegments: FoldSegment[],
): DepthLookup {
	const visibleHeights = [
		...new Set(visibleBlocks.map((block) => block.height)),
	].sort((a, b) => a - b);
	const columns: ColumnDescriptor[] = visibleHeights.map((height) => ({
		sortKey: height,
		kind: "height",
		height,
	}));
...
function buildVisibleBlockEdges(
	...
	for (const block of visibleBlocks) {
		...
		const highlightsSelected =
			...
			(selectedBlockId === block.id || selectedBlockId === block.prev_id);
...
function hasMineableActiveTip(
	block: ProcessedBlock,
	allNodes: NodeInfo[],
): boolean {
	const activeTip = block.tipStatuses.find(
	...
	return allNodes.some(
		(node) => node.supports_mining && activeTipNodeNames.has(node.name),
	);
}
...
function buildMineGraphElements(
	...
	for (const block of visibleBlocks) {
		...
		if (!occupiedSlotsAtDepth.has(targetDepth))
			occupiedSlotsAtDepth.set(targetDepth, []);
...
export function buildReactFlowGraph(
	...
	const slotByBlockId = compactSlots(
		createSlotLayout(blocks, blockMap, childrenMap),
		blocks,
		childrenMap,
	);
	const resolveNextMineSlot = createNextMineSlotResolver(
		slotByBlockId,
		childrenMap,
		blockMap,
	);
	...
	const { heightToDepth, foldToDepth } = createDepthLookup(
		visibleBlocks,
		collapsedSegments,
	);
	...
	const foldedNodes = buildFoldedNodes(
		collapsedSegments,
		foldToDepth,
		slotByBlockId,
	);
	const blockEdges = [
		...buildVisibleBlockEdges(
			visibleBlocks,
			blockMap,
			hiddenBlockIds,
			selectedBlockId,
