...
function App() {
	...
	const {
		...
		error: dataError,
		connectionStatus,
	} = useNetworkData(selectedNetworkId);
	...
	const selectedBlock = useMemo(() => {
		...
		return (
			processedBlocks.find((block) => block.id === selectedBlockId) ?? null
		);
	...
	useEffect(() => {
		if (
			...
			foldMeta.hiddenBlockIds.has(selectedBlockId)
		) {
			setSelectedBlockId(null);
		}
	}, [selectedBlockId, foldMeta.hiddenBlockIds]);

	const canShowSelectedBlockPanel =
		selectedBlock !== null &&
	...
	const reachableNodes =
		data?.nodes.filter((node) => node.reachable).length ?? 0;
