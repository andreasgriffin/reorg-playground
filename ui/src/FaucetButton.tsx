...
export function FaucetButton({
	...
	const eligibleNodes = useMemo(
		() =>
			nodes.filter((node) => node.supports_controls && node.supports_mining),
		[nodes],
	);
	...
	const selectedNode =
		eligibleNodes.find((node) => String(node.id) === nodeId) ?? null;
	const canSubmit =
		selectedNode !== null &&
		address.trim().length > 0 &&
		amountBtc.trim().length > 0 &&
		!loading;
			...
			<Dialog open={dialogOpen} onOpenChange={handleOpenChange}>
				...
				>
					<DialogHeader>
						...
						<DialogDescription>
							Send an unconfirmed wallet transaction from a selected regtest
							node without mining a confirmation block.
						</DialogDescription>
					</DialogHeader>
