...
export function useMineBlock(network: Network, nodes: MineControlNode[] = []) {
	...
	>(MINE_BLOCK_MUTATION_KEY, async (_key, { arg }) => {
		const result = await mineBlock(arg.networkId, arg.nodeId, {
			count: arg.count,
			excludeMempoolTransactions: arg.excludeMempoolTransactions,
		});
		if (!result.success) {
